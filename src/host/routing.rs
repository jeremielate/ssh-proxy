...
pub async fn add_route(tun_name: &str, subnet: IpAddr, prefix: u8) -> anyhow::Result<()> {
...
// future
#[allow(dead_code)]
pub async fn remove_route(tun_name: &str, subnet: IpAddr, prefix: u8) -> anyhow::Result<()> {
    let (connection, handle, _) = new_connection()?;
    tokio::spawn(connection);
    ...
    let route_message = match subnet {
        IpAddr::V4(_) => RouteMessageBuilder::<Ipv4Addr>::new().build(),
        IpAddr::V6(_) => RouteMessageBuilder::<Ipv6Addr>::new().build(),
    };
    ...
    while let Some(route) = routes.try_next().await? {
        ...
        if let Some((dest, test_if_index)) = get_destination_and_interface_attributes(&route) {
            let matches = match (dest, subnet) {
                (RouteAddress::Inet(dest_ip), IpAddr::V4(v4)) => *dest_ip == v4,
                (RouteAddress::Inet6(dest_ip), IpAddr::V6(v6)) => *dest_ip == v6,
                _ => false,
            };
            if matches
